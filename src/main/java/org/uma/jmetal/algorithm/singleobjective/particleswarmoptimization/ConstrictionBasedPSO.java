...
public class ConstrictionBasedPSO extends AbstractParticleSwarmOptimization<DoubleSolution, DoubleSolution> {
  ...
  public void initializeGlobalBest(List<DoubleSolution> swarm) {
    ...
    for (int i = 0; i < swarm.size(); i++) {
      if (globalBestParticle.getObjective(objectiveId) > swarm.get(i).getObjective(objectiveId)) {
    ...
    for (int i = 0; i < swarmSize; i++) {
      ...
      for (int var = 0; var < particle.getNumberOfVariables(); var++) {
        speed[i][var] = constrictionFactor * (weight * speed[i][var] +
                c1 * r1 * (localBest[i].getVariableValue(var) - particle.getVariableValue(var)) +
                c2 * r2 * (globalBestParticle.getVariableValue(var) - particle.getVariableValue
                        (var)));
      }
    }
  }
