...
public class proportional2 extends Clone<List<DoubleSolution>> {
	...
	public proportional2(int clonesize) {
	...
	public List<DoubleSolution> execute(List<DoubleSolution> parents) {
        ...
        for (k = 0; k < parents.size(); k++ ) {
            if (crowdingDistance.getAttribute(parents.get(k)).equals(Double.POSITIVE_INFINITY)) {
        ...
        for(k=0;k<parents.size();k++){
            clones[k]= (int)(clonesize*crowdingDistance.getAttribute(parents.get(k))/sum_distance)+1;
            if(sum_distance==0){
