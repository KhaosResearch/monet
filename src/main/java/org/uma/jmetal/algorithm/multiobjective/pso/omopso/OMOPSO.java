...
import org.uma.jmetal.algorithm.multiobjective.pso.omopso.util.EpsilonDominanceComparator;
...
public class OMOPSO extends AbstractParticleSwarmOptimization<DoubleSolution, List<DoubleSolution>> {
  ...
  private BoundedArchive<DoubleSolution> leaderArchive;
  ...
  public OMOPSO(DoubleProblem problem, SolutionListEvaluator<DoubleSolution> evaluator,
    ...
    leaderArchive = leaders ;
    epsilonArchive = new NonDominatedSolutionListArchive<DoubleSolution>(new EpsilonDominanceComparator<DoubleSolution>(epsilon));
  ...
  @Override public List<DoubleSolution> getResult() {
    if (variant.equals("epsilonArchive")) {
      return this.epsilonArchive.getSolutionList();
    } else if (variant.equals("crowdingArchive")){
      return this.leaderArchive.getSolutionList();
    } else {
      throw new JMetalException("Variant unknown: " + variant) ;
    }
  }

  @Override
  protected void initializeGlobalBest(List<DoubleSolution> swarm) {
  ...
  @Override
  protected void initializeParticleBest(List<DoubleSolution> swarm)  {
  ...
  protected void updateVelocity(List<DoubleSolution> swarm)  {
    ...
    for (int i = 0; i < swarmSize; i++) {
      ...
      if (leaderArchive.getComparator().compare(one, two) < 1) {
        bestGlobal = one ;
      } else {
        bestGlobal = two ;
      }
  ...
  @Override
  protected void updateParticleBest(List<DoubleSolution> swarm) {
    for (int i = 0; i < swarm.size(); i++) {
