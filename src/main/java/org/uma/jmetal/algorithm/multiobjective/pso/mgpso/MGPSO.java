package org.uma.jmetal.algorithm.multiobjective.pso.mgpso;
  ...
  private void updateSwarmVelocity(List<List<DoubleSolution>> swarmList) {
    for (ConstrictionBasedPSO pso : psoIslandList) {
      ...
      for (int i = 0; i < swarmSize; i++) {
        ...
        DoubleSolution one = archive.getSolutionList().get(pos1);
        DoubleSolution two = archive.getSolutionList().get(pos2);
        ...
        if (crowdingDistanceComparator.compare(one, two) < 1) {
          leader = (DoubleSolution) one.copy();
        } else {
          leader = (DoubleSolution) two.copy();
        }
        ...
        r3 = randomGenerator.nextDouble(0, 1.0);
        ...
        for (int var = 0; var < particle.getNumberOfVariables(); var++) {
          pso.getSwarmSpeedMatrix()[i][var] = weight * pso.getSwarmSpeedMatrix()[i][var] +
                  c1 * r1 * (localBest.getVariableValue(var) - particle.getVariableValue(var)) +
                  c2 * r2 * (globalBest.getVariableValue(var) - particle.getVariableValue(var));
        }
      }
    }
  }
